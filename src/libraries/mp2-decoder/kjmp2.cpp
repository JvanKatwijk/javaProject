...
unsigned long kjmp2_decodeFrame(
    kjmp2_context_t *mp2,
...
kjmp2_context_t *mp2 = reinterpret_cast <kjmp2_context_t *>(handler);
jbyte *inBody = env -> GetByteArrayElements	(indata, 0);
jshort  *outBody = env -> GetShortArrayElements	(outdata, 0);
int r	= kjmp2_decodeFrame (mp2, (const unsigned char *)inBody, outBody);
	env     -> ReleaseShortArrayElements	(outdata, outBody, 0);
	env     -> ReleaseByteArrayElements	(indata,  inBody, 0);
	return r != 0;
}
...
JNIEXPORT jlong JNICALL Java_package_1Model_MP2_1Processor_init_1mp2lib
   (JNIEnv *env , jobject obj) {
	...
	kjmp2_init (mp2);
	return reinterpret_cast <long>(mp2);
}
